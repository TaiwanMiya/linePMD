...
{
    public partial class Threadings : UserControl
    {
        private string EXE
        {
            get
            ...
            set
            {
                Material.SetExecute("thread", value);
            }
        }
        ...
        private void thread0x1_Click(object sender, EventArgs e)
        {
            if (!Material.IsLogin)
            {
            ...
            else
            {
                    ...
                    && Material.GroupsIndex > -1)
                {
                    ...
                    Material.ServoValues[1] = $"thread -a {count}";
        ...
        private void thread0x2_Click(object sender, EventArgs e)
        {
            if (!Material.IsLogin)
            {
                Material.ErrorMessage("請先登入!\nPlease login first!");
            ...
            else
            {
                ...
                {
                    Material.ServoValues[0] = Material.GroupsInfo[Material.GroupsIndex][1];
                    Material.ServoValues[1] = $"thread -l";
                    Material.ServoValues[2] = "0";
                    Material.ServoValues[3] = Material.PermissionInfo["owner"][Material.UserFromIndex];
                    ReadWrite.WriteInstruction();
                    Material.ServoValues = new string[4];
