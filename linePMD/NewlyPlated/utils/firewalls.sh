config_firewall(){
    ...
    if centosversion 6; then
        ...
        if [ $? -eq 0 ]; then
            ...
            _echo -w "iptables没有运行或没有安装,自行排查问题后,手动启用端口: ${PORT}."
        fi
        ...
        if [ $? -eq 0 ]; then
            ...
            _echo -w "firewalld没有运行或没有安装,自行排查问题后,手动启用端口: ${PORT}."
        fi
    else
        if [ "$(command -v ufw)" ] && [ -n "$(ufw status | head -n 1 | grep 'Status: active')" ]; then
            ...
            _echo -w "ufw没有运行或没有安装,自行排查问题后,手动启用端口: ${PORT}."
        fi
    fi
}
...
open_port_for_oracle_cloud(){
    ...
    if [ "$(command -v iptables)" ]; then
        ...
        _echo -w "iptables命令不存在,自行排查问题后,手动启用端口: ${PORT}."
    fi
}
