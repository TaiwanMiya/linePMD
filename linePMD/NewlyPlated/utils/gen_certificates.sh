...
intall_acme_tool(){
    ...
    if [ ! -e ${ACMESH_INSTALL_DIR}/acme.sh ]; then
        ...
        _echo -i "证书生成工具 acme.sh 已经安装，自动进入下一步，选择. "
    fi
}
...
get_input_api_info(){
    ...
    while true
    do
        _read "输入Cloudflare - Global API Key:"
        CF_Key="${inputInfo}"
        if [[ $(echo ${#CF_Key}) -ne 37 ]]; then
            _echo -e "输入正确合法的Global API Key."
            continue
        fi
...
_acme_cmd_by_manual(){
    ...
    if [ "${algorithmType}" == "RSA" ]; then
        ...
        if [[ $? -ne 0 && $? -ne 2 ]]; then
            _echo -i "根据上方提示,去Cloudflare上添加txt记录,完成后按任意键开始。"
            _echo -i "如果出现“too many certificates already issued for exact set of domains”错误,按Ctrl+C终止。"
...
acme_get_certificate_by_manual_force(){
    ...
    get_domain_ip "${domain}"
    if ! (echo "${domain}" | grep -qE '.cf$|.ga$|.gq$|.ml$|.tk$' && is_cdn_proxied "${domain_ip}"); then
        _echo -e "此选项为手动申Cloudflare CDN模式 证书(有效期3个月)，仅支持后缀为.cf .ga .gq .ml .tk的域名。"
        exit 1
    fi
    if [ -d "/root/.acme.sh/${domain}" ]; then
        ...
        _echo -e "在/root/.acme.sh/目录中找不到${domain}的申记录，不提供强制续期申。"
        exit 1
    fi
