other_status(){
    ...
    if [[ -e ${ssPath} ]] && [[ -e ${pluginPath} ]] && [[ -e ${webPath} ]]; then
        if [[ -n ${ssPid} ]] && [[ -n ${pluginPid} ]] && [[ -n ${webPid} ]]; then
            ...
            echo -e "\n${Point} ${ssName} is already installed but not running ^_^"
            echo -e "${Point} ${pluginName} is already installed but not running ^_^"
            echo -e "${Point} ${webName} is already installed but not running ^_^\n"
        fi
    elif [[ -e ${ssPath} ]] && [[ -e ${pluginPath} ]]; then
        if [[ -n ${ssPid} ]] && [[ -n ${pluginPid} ]]; then
            echo -e "\n${Info} ${ssName} (pid ${ssPid}) is already running >_<"
            echo -e "${Info} ${pluginName} (pid ${pluginPid}) is already running >_<\n"
        else
            echo -e "\n${Point} ${ssName} is already installed but not running >_<"
            echo -e "${Point} ${pluginName} is already installed but not running >_<\n"
        fi
