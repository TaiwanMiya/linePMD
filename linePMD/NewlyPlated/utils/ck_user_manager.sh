...
ck2_users_manager(){
    ...
    echo
    read -e -p "输入数字 [1-3]:" ck_user_opts
...
user_manager_by_uid(){
    if [ ! "$(command -v ck-server)" ]; then
        echo -e "\n${Error} 仅支持 ss + cloak 组合下使用,确认是否是以该组合形式运行.\n"
        exit 1
    fi
